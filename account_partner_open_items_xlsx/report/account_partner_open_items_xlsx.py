...
class AccountPartnerOpenItemsXlsx(models.AbstractModel):
    ...
    _description = "Open Journal Items per partner"
    ...
    def _get_ws_params(self, workbook, data, partners):  # noqa: C901
        ...
        select_extra, join_extra, where_extra = self.env[
            "res.partner"
        ]._xls_query_extra()
        ...
        query_start = (
            ...
            "ON pr.debit_move_id = l.id OR pr.credit_move_id = l.id " + join_extra
        ).format(wiz.date_at)
        ...
        if wiz.account_ids:
            account_selection = "AND a.id IN %s " % str(wiz.account_ids._ids).replace(
                ",)", ")"
            )
        else:
    ...
    AND (
      (l.full_reconcile_id IS NULL AND
            ...
            if lines:
                ...
                def lines_map(x):
                    if x["j_type"] in ["sale", "purchase"]:
                        ...
                        x.update({"docname": x["move_name"] or "*{}".format(x["m_id"])})

                list(map(lines_map, lines))
                lines.sort(
                    key=lambda x: (x["l_date"], x["docname"] and x["docname"].lower())
                )

                p_map = [
                    {"p_id": x["p_id"], "p_name": x["p_name"], "p_ref": x["p_ref"]}
                    for x in lines
                ]
                # sort with p_id None entries at the end
                p_map.sort(
                    key=lambda x: (not x["p_id"], x["p_name"] and x["p_name"].lower())
                )
                for p in p_map:
                    # remove duplicates while preserving list order
                    if p["p_id"] not in [x.get("p_id", None) for x in partners]:
                        partners.append(p)
                        partner_lines = [x for x in lines if x["p_id"] == p["p_id"]]
                        p.update({"lines": partner_lines})
                        original_amounts = [x["amount_original"] for x in partner_lines]
                        amount_original = reduce(lambda x, y: x + y, original_amounts)
                        amount_original = round(amount_original, digits)
                        p["amount_original"] = amount_original
                        reconciled_amounts = [
                            x["amount_reconciled"] for x in partner_lines
                        ]
        ...
        for report in reports:
            ...
            wl = self.env["res.partner"]._xls_open_items_overview_fields(report)
            col_specs = self._get_overview_template()
            col_specs.update(
                self.env["res.partner"]._xls_open_items_overview_template(report)
            )
            ws_params_overview = {
                "ws_name": report["title_short"],
            ...
            wl = self.env["res.partner"]._xls_open_items_details_fields(report)
            if wiz.add_currency:
                wl.extend(["currency", "amount_currency", "amount_currency_reconciled"])
            if wiz.add_reconcile:
                wl.append("rec_details")
            col_specs = self._get_details_template()
            col_specs.update(
                self.env["res.partner"]._xls_open_items_details_template(report)
            )
            ws_params_details = {
    ...
    def _get_report_title(self, data, wiz, report, report_type):
        report_date = fields.Date.to_string(wiz.date_at)
        s = "  -  "
        title = wiz.company_id.name + s + report_date + ": "
        ...
        template = {
            "partner": {
                ...
                "lines": {
                    "type": "string",
                    "value": self._render(
                        "p['p_id'] and (p['p_name'] or '-') " "or p['p_full_name']"
                    ),
                },
                "width": 44,
            },
            "partner_ref": {
                "header": {"type": "string", "value": self._("Partner Reference")},
                "lines": {"type": "string", "value": self._render("p['p_ref'] or ''")},
                "width": 22,
            },
            "amount_original": {
                "header": {
    ...
    def _overview_report(self, workbook, ws, ws_params, data, partners):
        ...
        orig_pos = "amount_original" in wanted_list and wanted_list.index(
            "amount_original"
        )
        rec_pos = "amount_reconciled" in wanted_list and wanted_list.index(
            "amount_reconciled"
        )
        for partner in report["partners"]:
            ...
            row_pos = self._write_line(
                ...
                render_space={"p": partner, "bal_formula": bal_formula},
    ...
    def _get_details_template(self):
        ...
        template = {
            "document": {
                "header": {"type": "string", "value": self._("Document")},
                "lines": {"type": "string", "value": self._render("l['docname']")},
                "totals": {
                    "type": "string",
            ...
            "sup_inv_nr": {
                "header": {"type": "string", "value": self._("Supplier Invoice No")},
                "lines": {
                    "type": "string",
            ...
            "origin": {
                "header": {"type": "string", "value": self._("Source Document")},
                "lines": {
                    "type": "string",
            ...
            "date_maturity": {
                "header": {"value": self._("Due Date")},
                "lines": {
                    "value": self._render("l['date_due']"),
                    "format": self.format_tcell_date_left,
                },
                "width": 12,
            },
            "description": {
                ...
                "lines": {"value": self._render("l['l_name']")},
                "width": 60,
            },
            "amount_original": {
                "header": {
            ...
            "amount_currency": {
                ...
                "lines": {
                    "value": self._render("l['currency'] and l['amount_currency']"),
                    "format": self.format_tcell_amount_right,
                },
                "width": 16,
            },
    ...
    def _details_report(self, workbook, ws, ws_params, data, partners):
        ...
        for partner in report["partners"]:
            ...
            for line in partner["lines"]:
                ...
                rec_cell = self._rowcol_to_cell(row_pos, rec_pos)
                bal_formula = orig_cell + "-" + rec_cell
                render_space = {"l": line, "bal_formula": bal_formula}
                if wiz.add_reconcile:
                    rec_details = ""
                    if line["prl_ids"]:
                        rec_details += ",".join([str(x) for x in line["prl_ids"]])
                    if line["fr_name"]:
