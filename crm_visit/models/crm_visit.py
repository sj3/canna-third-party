...
class CrmVisit(models.Model):
    ...
    date = fields.Datetime(string='Visit Datetime', required=True,
                           readonly=True)
    duration = fields.Integer(string='Duration', readonly=True,
                              help="Estimated duration of the "
                              "visit in minutes")
    visit_reason = fields.Many2one(comodel_name='crm.visit.reason',
                                   string='Reason', required=True,
                                   readonly=True)
    visit_reason_details = fields.Text(string='Purpose', readonly=True)
    visit_feeling = fields.Many2one(comodel_name='crm.visit.feeling',
                                    string='Feeling',
                                    readonly=True)
    ...
    def unlink(self):
        for visit in self:
            if visit.state != 'draft':
                raise UserError(_("Only visits in state 'draft'"
                                  " can be deleted. "))
