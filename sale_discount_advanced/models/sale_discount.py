...
class SaleDiscount(models.Model):
    ...
    def _calculate_discount(self, lines):
        ...
        for sol in lines:
            ...
            # Remark:
            # Only the 'disc_pct' value is used in the code calling this method.
            # We could hence simplify the code via result[sol] = disc_pct.
    ...
    def _get_rule_lines(self, rules, lines):
        ...
        res = {}
        for rule in rules:
            rule_lines = lines.filtered(lambda r: rule._sol_product_match(r))
            if rule.matching_extra != "none":
                ...
                rule_lines = rule_lines.filtered(lambda r: getattr(rule, method)(r))
            res[rule] = rule_lines
        return res
