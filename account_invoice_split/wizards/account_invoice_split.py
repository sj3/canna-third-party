...
class AccountInvoiceSplit(models.TransientModel):
    ...
    def split_invoice(self):
        ...
        self.ensure_one()
        active_invoice_id = self.env.context.get("active_id")
        ...
        if lines_to_move:
            lines_to_move.with_context(check_move_validity=False).write(
            ...
            for invoice in (old_invoice, new_invoice):
                ...
                if invoice.amount_total < 0:
                    raise UserError(
                        _("The amount of the resulting invoices must be > 0.")
                    )
            ...
            for model_name in order_models:
                if model_name in self.env.registry:
                    ...
                    orders = order_model.search(
                        [("invoice_ids", "in", old_invoice.ids)]
                    )
                    if orders:
