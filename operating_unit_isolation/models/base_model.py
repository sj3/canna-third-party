    ...
    def _register_hook(self, cr):
        def _ou_search(
            ...
            if "operating_unit_id" in self._fields:
                if (
                    context.get("record", {}).get("_field", False)
                    and "_name" in context["record"]
                    and self.pool.get(context["record"]["_name"])
                    and context["record"]["_field"]
                    in self.pool.get(context["record"]["_name"])._fields
                    and getattr(
                    ...
                    filter_ou = False
                    if len(isolation) == 2:
                        if isolation[0] == context["parent_record"]["_o2m"]:
                            filter_ou = context["parent_record"].get(
                                isolation[1], False
                            )
